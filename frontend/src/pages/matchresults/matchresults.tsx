...
const MatchResults: React.FC = () => {
    ...
    const handleAddMatchResults = () => {
        ...
        fetch('http://localhost:8080/api/matches', {
            ...
            .then((savedMatches) => {
                const mappedMatches = savedMatches.map((match: MatchDto) => ({
                    id: match.id,
                    teamA: { name: match.teamAName } as Team, 
                    teamB: { name: match.teamBName } as Team, 
                    teamAGoals: match.teamAGoals,
                    teamBGoals: match.teamBGoals
                }));
                
                setMatchResults([...matchResults, ...mappedMatches]);
                setNewMatchDetails('');
                setShowModal(false);
            })
